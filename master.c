...
void initSharedMem(memoria_condivisa * scacchiera){
	...
	for(i=0; i<SO_NUM_G; i++){
		...
		scacchiera->mosse[i] = SO_N_MOVES * SO_NUM_P;
	}
...
int main(){
	...
	unsigned int riga_bandierina, colonna_bandierina;
	...
	for(i = 0; i < 4800; i++){
		sops.sem_num = i;     /* check the 0-th semaphore */
	...
	while(numBandierina>0){
		if(scacchiera->scacchiera[scacchiera->rigaRand][scacchiera->colonnaRand].pedinaOccupaCella == 0){	
			if(numBandierina == 1)/*posizione libera da pedine*/
				scacchiera->scacchiera[scacchiera->rigaRand][scacchiera->colonnaRand].bandierina = punteggioBandierina + SO_ROUND_SCORE % SO_NUM_P;
			else
	...
	*/
	...
	/*exit(0);*/
	print_status();
	printf("MASTER: Il numero delle bandierine è : %u\n", scacchiera->numero_bandierine);
	#if 1
	while(scacchiera->numero_bandierine){
	/* devo ricevere la coda di messaggi da parte delle pedine (se sono state conquistate bandierine */
		while((num_bytes_bandierine = msgrcv(queue_id_bandierine, &my_msg_bandierine, sizeof(my_msg), getpid(), IPC_NOWAIT)) != -1){
			printf("[MASTER]: SONO ENTRATO NEL CICLO BANDIERINE\n");			
			if(num_bytes_bandierine >= 0) {/* dobbiamo incrementare il punteggio ai giocatori e rimuovere la bandierina */
				printf("[MASTER] : STO RICEVENDO DALLA CODA MESSAGGI DELLE PEDINE\n");
				...
				printf("[MASTER] Il giocatore è %d, la riga bandierina è %d, la colonna è %d, e il valore è %d\n", giocatore_bandierina, riga_bandierina, colonna_bandierina, valore_bandierina);
				for(i = 0; i < 3; i++){
					if(giocatore_bandierina == scacchiera->giocatori[i]){
						scacchiera->punteggio[i] += valore_bandierina; 	
						printf("[MASTER] punteggio aggiornato giocatore %d, = %d\n", i, scacchiera->punteggio[i]);	
					}		
				}
				scacchiera -> scacchiera[riga_bandierina][colonna_bandierina].bandierina = 0;
				printf("[MASTER] valore aggiornato bandierina %u\n", scacchiera -> scacchiera[riga_bandierina][colonna_bandierina].bandierina);
				for(i = 0; i < scacchiera -> numero_bandierine; i++){
					if(scacchiera->posBandierine[i][0] == riga_bandierina && scacchiera->posBandierine[i][1] == colonna_bandierina){
				...
				printf("[MASTER: ERRORE RICEZIONE CODA DI MESSAGGI BANDIERINE %5d %d\n", getpid(), errno);
				TEST_ERROR;
			}
		}
	...
	printf("HO STAMPATO LE BANDIERINE\n");
	print_status();
	#endif
